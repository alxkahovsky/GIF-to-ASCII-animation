...
num_key_frames = int(input('Input number of frames (integer): '))
...
def check_dirs(path):
    ...
    return (img_frames_root, ascii_frames_root, filename)


def make_frames(num_key_frames: int, data: tuple):
    img_frames_root = data[0]
    ascii_frames_root = data[1]
    filename = data[2]
    ascii_list = []
    ...
    return (ascii_list, filename)


def make_html(data: tuple):
    ascii_list = data[0]
    filename = data[1]
...
make_html(make_frames(num_key_frames, check_dirs(path)))
